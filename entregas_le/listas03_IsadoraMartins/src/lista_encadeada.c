...
struct Estudante le_RemoveChave(struct ListaEnc *lista, char *chave)
{
    ...
    if (le_ListaNaoInicializada(lista)) return e_removido;
    if (le_ListaVazia(lista)) return e_removido;
    ...
    if (atual == NULL)
    {
        ec_imprimirCor("Elemento não encontrado para remoção.\n", "R");
        return e_removido;
    }
    ...
    ec_imprimirCor("Elemento removido com sucesso!\n", "G");
    return e_removido;
}
